...
def main():
    ...
    parser.add_option("--ports",dest='ports',type='string',help='specify the target port seperated by comma')
    (options,args)=parser.parse_args()
    if(options.ip_address==None) | (options.ports==None):
        print('[-] You must specify a target ip address and target port')
        exit(0)
